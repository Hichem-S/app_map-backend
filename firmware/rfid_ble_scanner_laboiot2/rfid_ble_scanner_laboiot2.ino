...
// ════════════════════════════════════════════════════════════════════════════
//  BLE HIT QUEUE
//  BLE callback runs in a FreeRTOS task — PubSubClient is NOT thread-safe.
//  Hits are stored here and published from the main loop.
// ════════════════════════════════════════════════════════════════════════════

struct BleHit { String mac; String fingerprint; int rssi; };
static std::vector<BleHit> bleQueue;
static portMUX_TYPE         bleMux = portMUX_INITIALIZER_UNLOCKED;

void enqueueBleHit(const String& mac, const String& fp, int rssi) {
  portENTER_CRITICAL(&bleMux);
  bleQueue.push_back({ mac, fp, rssi });
...
class InventoryBleCallbacks : public BLEAdvertisedDeviceCallbacks {
  void onResult(BLEAdvertisedDevice dev) override {
    ...
    enqueueBleHit(mac, fingerprint, rssi);
  }
};
...
void loop() {
  ...
  // ── Drain BLE hit queue (safe — main loop context) ───────────────────────
  std::vector<BleHit> pending;
  portENTER_CRITICAL(&bleMux);
  pending.swap(bleQueue);
  portEXIT_CRITICAL(&bleMux);
  for (const BleHit& h : pending) {
    StaticJsonDocument<384> doc;
    doc["mac"]         = h.mac;
    doc["fingerprint"] = h.fingerprint;
    doc["rssi"]        = h.rssi;
    doc["room_id"]     = ROOM_ID;
    doc["reader_id"]   = READER_ID;

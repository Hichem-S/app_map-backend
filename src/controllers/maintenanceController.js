...
const createTask = async (req, res, next) => {
  try {
    const { product_id, title, description, priority = 'medium', assigned_to, scheduled_date } = req.body;
    if (!product_id || !title) {
    ...
    const result = await query(
      `INSERT INTO maintenance_tasks
         (product_id, created_by, assigned_to, title, description, priority, scheduled_date)
       VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id`,
      [product_id, req.user.id, assigned_to || req.user.id, title, description || null, priority, scheduled_date || null]
    );
...
module.exports = { getTasks, createTask, updateStatus, deleteTask };
